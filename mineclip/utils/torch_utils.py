...
def load_state_dict(objects, states, strip_prefix=None, strict=False):
    ...
    def _load(paths, obj):
        """
        Args:
            paths: nested path to the object
            obj: object to load state_dict
        """
...
def save_state_dict(obj, model_path: str, serial: int, type_name="", strip_prefix=None):
    """
    Args:
        strip_prefix: only save the keys that have the prefix, and strip it
    """
    if not _implements_method(obj, "state_dict"):
        raise ValueError(f"Object {type(obj)} does not support state_dict() method")
